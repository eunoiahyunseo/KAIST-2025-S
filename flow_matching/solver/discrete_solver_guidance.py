...
class MixtureDiscreteEulerSolver(Solver):
    ...
    def sample(
        ...
        verbose: bool = False,
        y_condition: Optional[Tensor] = None,
        ...
        **model_extras,
        ...
        with ctx:
            for i in range(n_steps):
                ...
                uncond_extras = model_extras.copy()
                p_1t_uncond = self.model(x=x_t, t=t.repeat(x_t.shape[0]), extra=uncond_extras)

                if guidance_strength > 0.0:
                    cond_extras = model_extras.copy()
                    cond_extras['label'] = y_condition
                    # print('y_condition: ', y_condition)
                    p_1t_cond = self.model(x=x_t, t=t.repeat(x_t.shape[0]), extra=cond_extras)

                    log_p_1t_uncond = torch.log(p_1t_uncond + eps)
                    log_p_1t_cond = torch.log(p_1t_cond + eps)
                    # print('p_1t_uncond: ', log_p_1t_uncond)
                    # print('p_1t_cond: ', log_p_1t_cond)

                    log_p_1t_guided = (1.0 - guidance_strength) * log_p_1t_uncond + guidance_strength * log_p_1t_cond
                    p_1t = F.softmax(log_p_1t_guided, dim=-1)
                else:
                    p_1t = p_1t_uncond
                ...
                if i == n_steps - 1:
                    ...
                    # Set u_t(x_t|x_t,x_1) = 0 -> 결국에는 자기 자신으로의 전이를 막기 위함. (intensity -> 총 유출량)
